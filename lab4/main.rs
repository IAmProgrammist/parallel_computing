...
impl NeuralNetwork {
    ...
    pub fn fit(
        ...
        for weight_index in 0..self._weights.len() {
            self._weights[weight_index] = 0.0;//rng.random_range(0.0..1.0);
        }
        ...
        let nodes_amount = self._weights.len();
        let objects_amount = train_data.len();
        ...
        for _epoch in 0..epochs {
            ...
            let kernel_event = unsafe {
                ExecuteKernel::new(&self._kernel_recalc_weights)
                    ...
                    .set_arg_local_buffer(objects_amount.next_power_of_two() * size_of::<f32>())
                    .set_local_work_size(objects_amount.next_power_of_two())
                    .set_global_work_size(objects_amount.next_power_of_two() * train_data[0].len())
    ...
    pub fn predict(&self, train_data: &Vec<Vec<f32>>) -> Result<Vec<f32>, String> {
        let mut train_data_aligned = vec![0f32; train_data[0].len() * train_data.len()];
        for i in 0..train_data.len() {
            for j in 0..train_data[0].len() {
                train_data_aligned[j + i * train_data[0].len()] = train_data[i][j];
    ...
    fn _predict(&self, data: &Vec<Vec<f32>>, train_data_aligned: &Vec<f32> ) -> Result<Vec<f32>, String> {
        ...
        let kernel_event = unsafe {
            ExecuteKernel::new(&self._kernel_prediction)
                ...
                .set_global_work_size(nodes_amount.next_power_of_two() * data.len())
                .enqueue_nd_range(&self._queue)?
        };
